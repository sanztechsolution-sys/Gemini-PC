...
async function initGeminiChat(windowElement: HTMLDivElement): Promise<void> {
    ...
    const sendMessage = async () => {
        ...
        try {
             // @ts-ignore
            const chat = geminiInstance.chats.create({ model: 'gemini-2.5-flash' });
             // @ts-ignore
...
async function critiquePaintDrawing(): Promise<void> {
    ...
    try {
        ...
        const textPart = { text: prompt };
         // @ts-ignore
        const result = await geminiInstance.models.generateContent({ model: "gemini-2.5-pro", contents: { parts: [ textPart, imagePart] } });
    ...
    async function getAiHint() {
        ...
        try {
            ...
            const prompt = `Minesweeper state:\n${boardState}\nShort, witty hint (1-2 sentences) for a safe move or dangerous area. Don't reveal exact mines unless certain. Hint:`;
             // @ts-ignore
            const result = await geminiInstance.models.generateContent({ model: "gemini-2.5-flash", contents: prompt, config: {temperature: 0.7}});
...
// --- Settings App Logic ---
function initSettingsApp(windowElement: HTMLDivElement): void {
    const generateBtn = windowElement.querySelector('#generate-wallpaper-btn') as HTMLButtonElement;
    const resetBtn = windowElement.querySelector('#reset-wallpaper-btn') as HTMLButtonElement;
    const promptInput = windowElement.querySelector('#wallpaper-prompt') as HTMLInputElement;
    const statusEl = windowElement.querySelector('#wallpaper-status') as HTMLParagraphElement;

    if (!generateBtn || !resetBtn || !promptInput || !statusEl) {
        console.error("Settings app UI elements not found.");
        return;
    }
    ...
    const generateWallpaper = async () => {
        ...
        if (!prompt) {
            statusEl.textContent = "Please enter a prompt for the wallpaper.";
            return;
        }

        statusEl.textContent = "Generating your wallpaper... This may take a moment.";
        generateBtn.disabled = true;
        resetBtn.disabled = true;

        try {
            if (!geminiInstance) {
                if (!await initializeGeminiIfNeeded('initSettingsApp')) {
                    throw new Error("Failed to initialize Gemini API.");
                }
            }
            // @ts-ignore
            const result = await geminiInstance.models.generateImages({
                model: 'imagen-4.0-generate-001',
                prompt: `A vibrant, high-resolution desktop wallpaper in the style of the 90s, inspired by: ${prompt}. Aspect ratio 16:9.`,
                config: {
                    numberOfImages: 1,
            ...
            statusEl.textContent = `Error: ${error.message || "Failed to generate wallpaper."}`;
            // If it fails, restore the default to avoid a broken state
    ...
    const resetWallpaper = () => {
        desktop.style.backgroundImage = 'none';
        desktop.style.backgroundColor = '#008080';
        statusEl.textContent = "Wallpaper reset to default.";
        promptInput.value = '';
    };
...
async function initializeGeminiIfNeeded(context: string): Promise<boolean> {
    ...
    try {
        ...
        if (!apiKey) {
            alert("CRITICAL ERROR: API Key missing.");
